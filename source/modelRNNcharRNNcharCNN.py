...
#		W_wemb = tf.Variable(tf.random_uniform([len_worddict, w_emb_size], -1.0, +1.0))
		...
		charlen= tf.reshape(self.word_len, [-1])				# NM
			...
			self.emb_rnn = tf.concat([states_fw, states_bw], 2 )		# NxMx20
		...
		with tf.variable_scope('char_cnn'):
			...
			self.emb_cnn = tf.reshape(h_pool, [-1, sentMax, num_filters_total])		#shape =(M, N, 210)
		...
		X = tf.concat([emb0, self.emb_cnn, self.emb_rnn, emb2], 2)				# (N, M, 130)
