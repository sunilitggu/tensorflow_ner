...
#		W_wemb = tf.Variable(tf.random_uniform([len_worddict, w_emb_size], -1.0, +1.0))
		...
		with tf.variable_scope('word_lstm'):
			...
			output_fw1 = outputs1				# NxMx100
			states_fw1 = states1
			h_rnn = output_fw1				# NxMx100
			h_rnn = tf.reshape(h_rnn, [-1, num_filters])	# NMx200	

		h_rnn = tf.nn.dropout(h_rnn, self.dropout_keep_prob)

		#Fully connected layer
		h_rnn = tf.tanh(h_rnn)
		W_1 = tf.get_variable("W_1", [num_filters, len_tagdict])		# 200x3
		b_1 = tf.Variable(tf.constant(0.1, shape=[len_tagdict]), name="b_1")	# 3

		scores = tf.nn.xw_plus_b(h_rnn, W_1, b_1, name="scores")		# NMx3

		self.pred = tf.argmax(scores, 1, name="predictions")			# NMx1
		self.predictions = tf.reshape(self.pred, [-1, sentMax]) 		# NxM

		self.correct_predictions = tf.equal(tf.cast(self.predictions, 'int32'), self.input_y)
		self.accuracy = tf.reduce_mean(tf.cast(self.correct_predictions, "float"), name="accuracy")

		input_y = tf.reshape(self.input_y, [-1])				# NM
		losses = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=scores, labels=input_y)	#

		self.loss = tf.reduce_mean(losses)  + l2_reg_lambda*(tf.nn.l2_loss(W_1)+tf.nn.l2_loss(b_1))
	...
	def train_step(self, t_w, t_p, t_t, t_s, t_ch, t_ch_len, drop_out=1.0):
		...
		feed_dict = {
   		...
   		_, loss, acc = self.sess.run([self.train_op, self.loss, self.accuracy], feed_dict)

    		print ("step "+str(self.step) + " loss "+str(loss) + " Acc "+str(acc) )
		return loss
	...
	def test_step(self, t_w, t_p, t_t, t_s, t_ch, t_ch_len, drop_out=1.0):
		...
		feed_dict = {
   		...
   		loss, pred, acc = self.sess.run( [self.loss, self.predictions, self.accuracy], feed_dict )
		print "Accuracy and loss in test set", acc, loss
		return acc, pred 
