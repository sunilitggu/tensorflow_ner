...
class WNN_CRF_NER(object):
	...
	def __init__(self, 
		...
		self.word  = tf.placeholder(tf.int32, [None, None, None], name="word")		# NxMxW
		...
		X_char = tf.reshape(X_char, [-1, charMax, c_emb_size])			# NMxCx25
		...
		if (charmodel == 'lstm'):
			...
			states_fw = tf.reshape(states_fw1.h, [-1, sentMax, cwes])	# NxMxWx10
			states_bw = tf.reshape(states_bw1.h, [-1, sentMax, cwes])	# NxMxWx10
			emb_c = tf.concat([states_fw, states_bw], 2 )			# NxMx20
			print 'emb_c', emb_c.get_shape()
			...
			h_pool = tf.concat(pooled_outputs,2)				#shape= (MNW, 1, 1, 20)			 
			emb_c = tf.reshape(h_pool, [-1, sentMax, num_filters_total])	#shape =(M, N, 20)

		
		#lookup layer
		emb0 = tf.nn.embedding_lookup(W_wemb, self.word)			# NxMxWx100
		emb0 = tf.reshape(emb0, [-1, sentMax, len_window*w_emb_size])		# NxMxW100

		emb2 = tf.nn.embedding_lookup(W_pemb, self.pos)				# NxMx10

		if charmodel == 'cnn' or charmodel == 'lstm':
			len_fd = len_window * w_emb_size + pes + 2*cwes
			X = tf.concat([emb0, emb_c, emb2], 2)				# (N, M, 330)
			X = tf.reshape(X, [-1, len_fd])	# (NM, 330)
			
		else:
			len_fd = len_window * w_emb_size + pes
			X = tf.concat([emb0, emb2], 2)					# (N,M,310)
			X = tf.reshape(X, [-1, len_fd])					# (NM,310)

		print 'X ', X.get_shape()

 
		#Layer-I
		W_1 = tf.get_variable("W_1", [len_fd, num_filter])			# 330xnum_filter
		b_1 = tf.Variable(tf.constant(0.1, shape=[num_filter]), name="b_1")	# num_filter
		h_1 = tf.nn.xw_plus_b(X, W_1, b_1, name="h_1")				# NMxnum_filter

		h_1 = tf.tanh(h_1)
		h_1 = tf.nn.dropout(h_1, self.dropout_keep_prob)

		#Layer-II
		W_2 = tf.get_variable("W_2", [num_filter, len_tagdict])			# 200x3
		b_2 = tf.Variable(tf.constant(0.1, shape=[len_tagdict]), name="b_2")	# 3
		scores = tf.nn.xw_plus_b(h_1, W_2, b_2, name="scores")			# NMx3
		...
		# Add a training op to tune the parameters.
    		self.loss = tf.reduce_mean(-self.log_likelihood) + l2_reg_lambda*(tf.nn.l2_loss(W_1)+tf.nn.l2_loss(b_1) + tf.nn.l2_loss(W_2)+tf.nn.l2_loss(b_2) )

#		self.train_op = tf.train.GradientDescentOptimizer(1).minimize(self.loss)
	...
	def test_step(self, t_w, t_p, t_t, t_s, t_ch, t_ch_len, drop_out=1.0):
		...
		print "Acc in test set", accuracy
